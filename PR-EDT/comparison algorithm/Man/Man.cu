...
__global__ void img_log(unsignimg_ed char *nums, int cols, int rows){
...
double* fun1(bool* flag,int cols,int rows){
	GPoint* nums1 = (GPoint*)malloc(sizeof(GPoint) * cols * rows); 
	GPoint* nums2 = (GPoint*)malloc(sizeof(GPoint) * cols * rows); 
	int nums1_size = 0;
	int nums2_size = 0; 
...
void tranformation(vector<vector<bool>>& flag) {  
	...
	int* lpoint;
	cudaMalloc((void **)&lpoint,block * len * sizeof(int));
	int* rpoint;
	cudaMalloc((void **)&rpoint,block * len * sizeof(int));
	double* l;
	cudaMalloc((void **)&l,rows * len * sizeof(double));
	double* r;
	cudaMalloc((void **)&r,rows * len * sizeof(double));
	int* temp;
	cudaMalloc((void **)&temp,block * len * sizeof(int));
...
int main(int argc, char **argv) {
	...
	for(int i = 0;i < rows.size();i++){
		...
		for(int k = 0;k < rows[i];k++)
			flag[k].resize(cols[i]);
		for(int j = 0;j < r.size();j++){
			
			for(int k = 0; k < rows[i]; k++){
				for(int l = 0;l < cols[i];l++)
					flag[k][l] = (rand() % 100) < r[j];
